...
expect_known_output(prov.trace(c("script-1.R", "script-2.R"), prov.dir=prov.dir, file.details=TRUE, check=FALSE), test.expected, update=FALSE)

# test prov.trace.run
test.expected <- system.file("testexpected", "prov.trace.run.expected", package="provTraceR", mustWork=TRUE)
expect_known_output(prov.trace.run(c("script-1.R", "script-2.R"), prov.dir=prov.dir, check=FALSE), test.expected, update=FALSE)
...
expect_known_output(prov.trace("script-3.R", prov.dir=prov.dir, file.details=TRUE, check=FALSE), test.expected, update=FALSE)
